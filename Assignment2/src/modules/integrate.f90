...
contains
    ...
    function simpson(d, f, a, b, dx, N) result(integral)
        ...
        real(8) :: integral, dx_(d)
        integer :: i, j, lattice(d), N_(d), weight
        ...
        do i = 1, product(N_ + 1)
            ...
            weight = 2.0d0 ** (count(mod(lattice, 2)==1) + d - count(mod(lattice, N_)==0))

            integral = integral + f(a + lattice * dx_) * weight
        end do

        integral = integral * product(dx_) / (3.0d0 ** d)
    ...
    function simpson2(d, f, a, b, dx, N) result(integral)
        ...
        do i = 1, product(N_ + 1)
            ...
            weight = 1.50d0 ** (count(mod(lattice, 3)/=0)) * 2.0d0 ** (d - count(mod(lattice, N_)==0))

            integral = integral + f(a + lattice * dx_) * weight
        end do

        integral = integral * product(dx_) * (0.3750d0 ** d)
