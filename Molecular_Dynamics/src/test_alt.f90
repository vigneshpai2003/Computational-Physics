program scratch
    ...
    call calc_neighbors_alt(N, x, neighbors_size, neighbors, R)

    call force_multi_alt(N, x, F, neighbors_size, neighbors, PE)

    do i = 1, N_t
        call update_x(N, x, v, F, dt)

        call force_multi_alt(N, x, F_new, neighbors_size, neighbors, PE)
