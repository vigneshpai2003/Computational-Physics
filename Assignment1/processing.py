...
def q_random():
    ...
    for bin_size in [0.5, 1, 2]:
        ...
        plt.savefig(f'figures/1h_dx_{str(bin_size).replace(".", "_")}.png')
        plt.clf()
...
def q_random_walks():
    ...
    for bin_size in [1, 2, 5, 10]:
        ...
        plt.ylabel('Normalized count')
        plt.title(f'Bin Size = {bin_size}')
        plt.savefig(f'figures/1ij_dx_{bin_size}.png')
        plt.clf()    


    plt.hist(x1, np.arange(min(x1) - min(x1) % 2, max(x1), 2), density=True, alpha=0.5)
    plt.hist(x1, np.arange(min(x1) - min(x1) % 2 - 1, max(x1), 2), density=True, alpha=0.5)
    plt.xlabel('Random walk of $10^4$ steps')
    plt.ylabel('Normalized count')
    plt.title(f'Bin Size = 2')
    plt.savefig(f'figures/1ij_shifted.png')
    plt.show()
    plt.clf() 
